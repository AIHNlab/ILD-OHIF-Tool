...
function commandsModule({
  ...
  const actions = {
    ...
    deleteMeasurement: ({ uid }) => {
      ...
      // uid here is always the underlying annotationUID (see
      // showCornerstoneContextMenu's selectorProps), but measurementService
      // only knows about tools it has a mapping for. Tools that never go
      // through that mapping (e.g. MONAI Label's own prompt/exclude
      // annotation tools) have no Measurement to remove, so
      // measurementServiceSource.remove() above would silently no-op and
      // leave the annotation on screen forever - fall back to removing the
      // raw cornerstone annotation directly.
