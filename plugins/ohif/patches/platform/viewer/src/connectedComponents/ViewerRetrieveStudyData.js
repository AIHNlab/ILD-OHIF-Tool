...
function ViewerRetrieveStudyData({
  ...
  const processStudies = (studiesData, filters) => {
    if (Array.isArray(studiesData) && studiesData.length > 0) {
      ...
      // Append rather than replace: loadStudies() now calls processStudies()
      // once per study (first study first, the rest sequentially after),
      // instead of once for the whole batch, so each call must add to
      // whatever's already loaded rather than overwrite it. This is also
      // correct for the very first call, since it appends onto the initial
      // empty array.
      setStudies(prevStudies => [...prevStudies, ...studies]);
    }
  };
  ...
  const loadStudies = async () => {
    ...
    const buildRetrieveParams = uids => {
    ...
    // One study's metadata retrieval + processing, tracked individually so
    // purgeCancellablePromises() can still cancel any of them independently.
    const retrieveAndProcess = async uid => {
      try {
        const cancelable = makeCancelable(
          retrieveStudiesMetadata(...buildRetrieveParams([uid]))
        );
        cancelableStudiesPromises[uid] = cancelable;
    ...
    // Load the first (primary) study fully before starting on the rest, so
    // it isn't competing for bandwidth/CPU with every other selected study's
    // metadata and pixel prefetching all starting at once. The remaining
    // studies then load one at a time, in the background, in URL order.
    const [firstStudyInstanceUID, ...remainingStudyInstanceUIDs] = studyInstanceUIDs;

    await retrieveAndProcess(firstStudyInstanceUID);

    for (const uid of remainingStudyInstanceUIDs) {
      await retrieveAndProcess(uid);
    }
  };
