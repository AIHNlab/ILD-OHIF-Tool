...
async function getStudyList(
  ...
  const mappedStudies = studies.map(study => {
    ...
    return {
      ...
      numberOfStudyRelatedInstances: study.numberOfStudyRelatedInstances, // "3"
...
function _sortStudies(studies, field, order) {
  ...
  const sortedStudies = studies.map(study => {
    if (
      study.StudyDate &&
      !moment(study.StudyDate, 'MMM DD, YYYY', true).isValid()
    ) {
      const reformatted = moment(study.StudyDate, 'YYYYMMDD').format(
        'MMM DD, YYYY'
      );
      // Source DICOM had a StudyDate tag but no actual value (common in
      // anonymized/de-identified datasets) -- don't show the literal
      // "Invalid date" string, leave it empty so the UI shows "(Empty)".
      study.StudyDate = reformatted === 'Invalid date' ? '' : reformatted;
    }
    return study;
  });
