...
function StudyList(props) {
  ...
  const largeTableMeta = [
    ...
    {
      displayText: t('Instances'),
      fieldName: 'numberOfStudyRelatedInstances',
      size: 100,
    },
    {
...
function TableRow(props) {
  ...
  const largeRowTemplate = (
    ...
    >
      ...
      <td className={classNames({ 'empty-value': !PatientID })}>
        {PatientID || `(${t('Empty')})`}
      </td>
      <td className={classNames({ 'empty-value': !AccessionNumber })}>
        {AccessionNumber || `(${t('Empty')})`}
      </td>
      <td className={classNames({ 'empty-value': !StudyDate })}>
        {StudyDate || `(${t('Empty')})`}
      </td>
      <td className={classNames({ 'empty-value': !modalities })}>
        {modalities || `(${t('Empty')})`}
      </td>
      <td style={{ textAlign: 'center' }}>{numberOfStudyRelatedInstances}</td>
      <td className={classNames({ 'empty-value': !StudyDescription })}>
        {StudyDescription || `(${t('Empty')})`}
      </td>
    </tr>
  );
    ...
    >
      ...
      <td>
        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
          {/* DESCRIPTION */}
          <div
            className={classNames('hide-xs', {
              'empty-value': !StudyDescription,
            })}
            style={{
              whiteSpace: 'pre-wrap',
              flexGrow: 1,
      ...
      <td
        ...
        className={classNames({ 'empty-value': !StudyDate })}
      >
        {StudyDate || `(${t('Empty')})`}
      </td>
    </tr>
      ...
      <td style={{ position: 'relative', overflow: 'hidden' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
          ...
          >
            ...
            <div>{StudyDate || `(${t('Empty')})`}</div>
            {numberOfStudyRelatedInstances && (
              <div style={{ fontSize: '11px', color: '#60656f' }}>
                {numberOfStudyRelatedInstances} {t('Instances')}
              </div>
            )}
          </div>
        </div>
