...
function TableSearchFilter(props) {
  ...
  return translationsAreReady
    ? meta.map((field, i) => {
        ...
        // The row-selection column has no label/sort concept of its own -
        // it gets a real "select all" checkbox instead of the usual
        // label+sort-icon+input layout the other columns share.
        if (fieldName === 'select') {
          return (
            <th
              key={`${fieldName}-${i}`}
              className="studylist-select-header"
            ...
            >
              <input
                ...
                aria-label={t('Select all studies on this page')}
              />
            </th>
          );
        }
        ...
        return (
          <th key={`${fieldName}-${i}`}>
            ...
            {/* Columns without a filter input (e.g. Instances) still need a
                same-height second row so their label stays pinned to the top
                like every other column's, instead of centering across the
                full header height when it's the label's only child. */}
            {!inputType && (
              <input
                type="text"
                className="form-control studylist-search"
...
TableSearchFilter.propTypes = {
  meta: PropTypes.arrayOf(
    PropTypes.shape({
      ...
      inputType: PropTypes.oneOf(['text', 'date-range']),
